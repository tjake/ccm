...
class Node(object):
    ...
    def watch_log_for_alive(self, nodes, from_mark=None, timeout=300, filename='system.log'):
        """
        Watch the log of this node until it detects that the provided other
        nodes are marked UP. This method works similarly to watch_log_for_death.
        """
        tofind = nodes if isinstance(nodes, list) else [nodes]
        tofind = ["%s.* %s" % (node.address(), ("now UP" if node.get_cassandra_version() < '3.0.16' else 'state NORMAL')) for node in tofind]
        self.watch_log_for(tofind, from_mark=from_mark, timeout=timeout, filename=filename)
